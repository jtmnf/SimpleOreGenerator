...
public class RetroGenWorld {
    ...
    public void chunkLoadEvent(ChunkDataEvent.Load event) {
        ...
        NBTTagCompound tagCompound = (NBTTagCompound) event.getData().func_74781_a(nbtModId);

        boolean retronGen = false;
        if(tagCompound != null){
            boolean configRetroGen = ConfigHandler.retrogenBedrockAccess || ConfigHandler.retrogenOresAccess;

            if(configRetroGen && !tagCompound.func_74764_b(nbtGenerated)){
                retronGen = true;
            }
        }
        ...
        if (retronGen) {
            ...
            if (chunks != null) {
                chunks.add(new ChunkInfo(event.getChunk().field_76635_g, event.getChunk().field_76647_h));
                retroChunk.put(dimensionID, chunks);
            }
        }
    }
    ...
    public void chunkSaveEvent(ChunkDataEvent.Save event){
        NBTTagCompound tagCompound = event.getData().func_74775_l(nbtModId);
        if(!tagCompound.func_74764_b(nbtGenerated)){
            tagCompound.func_74757_a(nbtGenerated, true);
        }
        event.getData().func_74782_a(nbtModId, tagCompound);
    }
}
